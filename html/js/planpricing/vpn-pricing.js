        ...
        renderPricingPage: (plan, $parentPage, callback) => {
            ...
            return pro.proplan2.feature.renderPricingPage(
                ...
                opts.map((opt) => {
                    ...
                    const [,,,,months,price,currency,montlyBasePrice,localPrice,localCurrency] = opt;
                    const hasLocal = !!localCurrency;
                    let priceMonthly = hasLocal ? localPrice : price;
                    ...
                    const feature = {
                        // btnText: plan[pro.UTQA_RES_INDEX_EXTRAS].trial
                        //     ? mega.icu.format(l.try_free_for_days, plan[pro.UTQA_RES_INDEX_EXTRAS].trial.days)
                        //     : l.pr_buy_vpn;
                        btnText: mega.icu.format(l.try_free_for_days, 7),
                        clickName: 'vpnPricing',
                        title: pro.propay.getRecurringDurationWording(months),
                        classes,
                        hasLocal,
                        priceCurrency: hasLocal ? localCurrency : currency,
                        priceMonthly,
                        onclick: callback.bind(null, months, opt[pro.UTQA_RES_INDEX_ACCOUNTLEVEL])
                    };
                    ...
                    if (dealHeader) {
                        feature[dealHeader] = l.yearly_plan_saving.replace(
                            '%1',
                            percentageDiff(price / months, montlyBasePrice, 3)
                        );
                    }
