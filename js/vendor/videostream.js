 ...
 * $Id: videostream.js,v 2.4.2 2018/01/30 05:35:03 dc Exp $
 */
...
MP4Remuxer.prototype._parseMoov = function() {
  ...
  if (t) {
    ...
    try {
      ...
      t.message = "Cannot parse mp4 file: " + t.message;
      e.emit("error", t);
    }
  }
};
...
MP4Remuxer.prototype._findMoov = function(e) {
  ...
  t._decoder.once("box", function(r) {
    var i = function() {
      t._decoder.destroy();
      t["_" + n] ? t._parseMoov() : t._findMoov(e + r.length);
    };
    a.destroy();
...
MP4Remuxer.prototype._processMoov = function(e) {
  ...
  for (var s = 0; s < r.length; s++) {
    var n, a, i = r[s], o = i.mdia.minf.stbl, m = o.stco || o.co64, f = o.stsd.entries[0], c = i.mdia.hdlr.handlerType;
    d && console.debug("handler=%s, type=%s, trak:", c, f.type, i);
    if ("vide" === c && "avc1" === f.type) {
      if (t._hasVideo) continue;
      t._hasVideo = !0;
      n = "avc1";
      f.avcC && (n += "." + f.avcC.mimeCodec);
      a = 'video/mp4; codecs="' + n + '"';
      d && console.debug(a);
    } else {
      if ("soun" !== c || "mp4a" !== f.type) continue;
      if (t._hasAudio) continue;
      n = "mp4a";
      f.esds && f.esds.mimeCodec && (n += "." + f.esds.mimeCodec);
      a = 'audio/mp4; codecs="' + n + '"';
      d && console.debug(a);
      if (!MediaSource.isTypeSupported(a)) continue;
      t._hasAudio = !0;
    }
    var u = [], l = 0, p = 0, h = 0, v = 0, _ = 0, g = 0, y = new RunLengthIndex(o.stts.entries), x = null;
    o.ctts && (x = new RunLengthIndex(o.ctts.entries));
    for (var S = 0; ;) {
      var k = o.stsc.entries[_], M = o.stsz.entries[l], b = y.value.duration, I = !0;
      o.stss && (I = o.stss.entries[S] === l + 1);
      u.push({
        size: M,
        duration: b,
        dts: g,
        presentationOffset: x ? x.value.compositionOffset : 0,
        sync: I,
        offset: v + m.entries[h]
      });
      if (++l >= o.stsz.entries.length) break;
      v += M;
      if (++p >= k.samplesPerChunk) {
        p = 0;
        v = 0;
        h++;
        var R = o.stsc.entries[_ + 1];
        R && h + 1 >= R.firstChunk && _++;
      }
      g += b;
      y.inc();
      x && x.inc();
    ...
    i.mdia.mdhd.duration = 0;
    i.tkhd.duration = 0;
    var B = k.sampleDescriptionId, E = {
      type: "moov",
      mvhd: e.mvhd,
      traks: [ {
        tkhd: i.tkhd,
      ...
      mvex: {
        ...
        trexs: [ {
          ...
          defaultSampleDescriptionIndex: B,
...
MP4Remuxer.prototype._writeFragment = function(e, t) {
  var r = this, s = r._tracks[t], n = s.inStream, a = s.outStream, i = function(e) {
    return function(t) {
      t ? r.emit("error", t) : a.destroyed || n.destroyed ? d > 1 && console.debug("writeFragment stream destroyed", a.destroyed, n.destroyed) : e();
    };
  }, o = i(function() {
    if (!(e = r._generateFragment(t))) return a.finalize();
    f();
  }), m = i(function() {
    var t = n.slice(e.ranges), r = a.mediaData(e.length, o);
    t.pipe(r);
  }), f = function() {
    a.destroyed || a.box(e.moof, m);
  };
  f();
};
...
MP4Remuxer.prototype.seek = function(e) {
  ...
  t._tracks.forEach(function(n, a) {
    n.fragment = t._generateFragment(a, e);
    if (n.fragment) {
      var i = n.fragment.ranges[0].start;
      s = Math.min(s, i);
      r = r ? Math.abs(r - i) : i;
    }
  });
  ...
  return t._tracks.map(function(n, a) {
    n.outStream && n.outStream.destroy();
    if (n.inStream) {
      n.inStream.destroy();
      n.inStream = null;
    }
    n.outStream = mp4.encode();
    if (n.fragment) {
      d && console.debug("MP4Remuxer.seek(%s)", e, n, s);
      n.inStream = new RangeSliceStream(s, {
        highWaterMark: r
      });
      t._fileStream || (t._fileStream = t._file.createReadStream({
        start: s
      }));
      t._fileStream.pipe(n.inStream);
      t._writeFragment(n.fragment, a);
    } else n.outStream.finalize();
    return n.outStream;
  });
};
...
MP4Remuxer.prototype._generateMoof = function(e, t, r) {
  for (var s = this._tracks[e], n = [], a = t; a < r; a++) {
    var i = s.samples[a];
    n.push({
...
Box.boxes.sidx.decode = function(e, t) {
  var r, s = Object.create(null), n = t + 4, a = function() {
    var t = e.readUInt16BE(n);
    n += 2;
    return t;
  }, i = function() {
    var t = e.readUInt32BE(n);
    n += 4;
    return t;
  }, o = function() {
    var e = i(), t = i();
...
exports.Descriptor.decode = function(r, e, o) {
  var t, c, s = r.readUInt8(e), p = e + 1, i = 0;
  do {
    i = i << 7 | 127 & (t = r.readUInt8(p++));
  } while (128 & t);
  var n = tagToName[s];
  (c = exports[n] ? exports[n].decode(r, p, o) : {
    buffer: new Buffer(r.slice(p, p + i))
  }).tag = s;
  c.tagName = n;
  c.length = p - e + i;
  c.contentsLen = i;
...
Box.readHeaders = function(e, t, o) {
  t = t || 0;
  if ((o = o || e.length) - t < 8) return 8;
  var n, s, r = e.readUInt32BE(t), i = e.toString("ascii", t + 4, t + 8), f = t + 8;
  if (1 === r) {
    if (o - t < 16) return 16;
    r = uint64be.decode(e, f);
    f += 8;
  }
  d > 1 && console.warn("Box.readHeaders", [ e ], t, o, i, containers[i], boxes[i], boxes.fullBoxes[i]);
  "sidx" === i && (f -= 8);
  if (boxes.fullBoxes[i]) {
    n = e.readUInt8(f);
    s = 16777215 & e.readUInt32BE(f);
    f += 4;
  }
  return {
    length: r,
...
Box.decodeWithoutHeaders = function(e, t, o, n) {
  ...
  if (containers[s]) {
    r.otherBoxes = [];
    for (var i = containers[s], f = o; n - f >= 8; ) {
      var a = Box.decode(t, f, n);
      f += a.length;
      if (i.indexOf(a.type) >= 0) r[a.type] = a; else if (i.indexOf(a.type + "s") >= 0) {
        var c = a.type + "s";
        (r[c] = r[c] || []).push(a);
...
VideoStream.prototype._unsafePump = function() {
  ...
  e._tracks.forEach(function(t, o) {
    var i = function() {
      if (t.muxed) {
        t.muxed.destroy();
    ...
    t.initFlushed ? i() : t.onInitFlushed = function(r) {
      r ? e._elemWrapper.error(r) : i();
    };
  });
};
