...
function mfolder(id)
{
	...
	if ((h) && (h.get('folder')))
	{
		...
		if (mfolderinfo.length == 11) icon = staticpath +  'images/extension_large/contact@2x.png';
		else if (sharednodes[h.get('id')]) icon = staticpath +  'images/extension_large/foldershared.png';
	...
	else
	{
		...
		if (h)
		{
			...
			api_req([{	
				a: 'p',
				t: RootID,
				n: [{ h: h.get('id'), t: 0, a: h.get('attrs'), k: a32_to_base64(encrypt_key(u_k_aes,h.get('key'))) }]
			}],
			{
			callback: function()
			{
...
function mstartdl()
{
	...
	if (h)
	{
		...
		{
			if (mobile_filesupported_coming(h.get('name'))) alert(l[805]);
			else alert(l[804]);
		} 
	}
}
...
function mfile(id)
{
	...
	{
		mfileinfo=id;		
		var icon = fileicon(h.get('name'),0).replace('extension_medium','extension_large');
		var thumbstyle = '';		
...
function mback()
{
  ...
  else if (!currentdirid)
  {
	...
	if (!fminitialized) document.location.hash = '#start';
	else opendirectory(RootID);
  }
  else
  {
...
function m_attr_events()
{
	...
	{	
		console.log(e);
		var action = e.target.id.replace('datatitle_','').split('_')[0];
		var id = e.target.id.replace('datatitle_','').split('_')[1];			
...
{		
	...
	if (!lightweight && !fmrendered) renderfm();
	try
	{
	...
	if (!mfmloaded && !lightweight) init_mfm(); 
	$('.top-trash-icon').hide();
	...
	if (((typeof currentdirid != 'undefined' && ((currentdirid == RootID) || (currentdirid == TrashbinID) || (currentdirid == NetworkID) || (currentdirid == InboxID)) && is_fm()) || (page == 'pro') || (page == 'about') || (page == 'blog') || (page == 'account') || (page == 'privacy') || (page == 'terms') || (page == 'help')) && !mfileinfo && !mfolderinfo)
	{
		$('#mmenubtn').show();
		$('#mbackbtn').hide();
	}
	...
	{	
		var icon = fileicon(fdl_file.n,0).replace('extension_medium','extension_large');
		var thumbstyle = '';
	...
	{		
		if (RootbyId(currentdirid) == NetworkID)
		{
			...
			else if ((h) && (h.get('parentid') == NetworkID)) 
			{
				$('.top-add-icon').hide();
				$('#mobile_title').text(Ext.util.Format.ellipsis(h.get('name'),20));
			}
		...
		else if (RootbyId(currentdirid) == TrashbinID) $('#mobile_title').text(l[167]);
...
{
	if (page == 'download') $('#preview-title').text(Ext.util.Format.ellipsis(fdl_file.n,20));
	else
	{
		var h = FileStore.getById(mfileinfo);
		if (h) $('#preview-title').text(Ext.util.Format.ellipsis(h.get('name'),20));
...
function mnewcontact()
{
	...
	{
		loadingDialog.show();
		processcontact($('#mobile_newcontact').val());
	}
}
...
{		
	...
	$("#mobile_link").bind('click',function() 
	{
		$(this)[0].setSelectionRange(0,9999);
	});	
	...
	{		
		...
		else 	
		{
			...
			if (currentdirid == NetworkID) mclosecontact();			
