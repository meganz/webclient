 ...
 * $Id: videostream.js,v 2.4.2 2018/01/26 00:15:29 dc Exp $
 */
...
MP4Remuxer.prototype._findMoov = function(e) {
  ...
  r.pipe(t._decoder);
  t._decoder.once("box", function(n) {
    d && console.debug("box", n.type, n.length, n, e);
    if ("moov" === n.type) t._decoder.decode(function(e) {
      r.destroy();
      try {
        t._processMoov(e);
      } catch (e) {
...
MP4Remuxer.prototype._processMoov = function(e) {
  ...
  for (var n = 0; n < r.length; n++) {
    var a, s, i = r[n], o = i.mdia.minf.stbl, m = o.stco || o.co64, f = o.stsd.entries[0], c = i.mdia.hdlr.handlerType;
    d && console.debug("handler=%s, type=%s, trak:", c, f.type, i);
    if ("vide" === c && "avc1" === f.type) {
      if (t._hasVideo) continue;
      t._hasVideo = !0;
      a = "avc1";
      f.avcC && (a += "." + f.avcC.mimeCodec);
      s = 'video/mp4; codecs="' + a + '"';
      d && console.debug(s);
    } else {
      if ("soun" !== c || "mp4a" !== f.type) continue;
      if (t._hasAudio) continue;
      a = "mp4a";
      f.esds && f.esds.mimeCodec && (a += "." + f.esds.mimeCodec);
      s = 'audio/mp4; codecs="' + a + '"';
      d && console.debug(s);
      if (!MediaSource.isTypeSupported(s)) continue;
      t._hasAudio = !0;
    }
    var u = [], l = 0, p = 0, h = 0, _ = 0, v = 0, g = 0, y = new RunLengthIndex(o.stts.entries), S = null;
    o.ctts && (S = new RunLengthIndex(o.ctts.entries));
    for (var x = 0; ;) {
      var k = o.stsc.entries[v], M = o.stsz.entries[l], b = y.value.duration, R = !0;
      o.stss && (R = o.stss.entries[x] === l + 1);
      u.push({
        size: M,
        duration: b,
        dts: g,
        presentationOffset: S ? S.value.compositionOffset : 0,
        sync: R,
        offset: _ + m.entries[h]
      });
      ...
      if (++p >= k.samplesPerChunk) {
        ...
        _ = 0;
        h++;
        var I = o.stsc.entries[v + 1];
        I && h + 1 >= I.firstChunk && v++;
      }
      g += b;
      y.inc();
      S && S.inc();
  ...
  if (0 !== t._tracks.length) {
    ...
    var w = Box.encode(t._ftyp), P = t._tracks.map(function(e) {
      var t = Box.encode(e.moov);
      return {
        mime: e.mime,
        init: Buffer.concat([ w, t ])
      };
    });
    t.emit("ready", P);
...
MP4Remuxer.prototype._writeFragment = function(e, t) {
  var r = this, n = r._tracks[t], a = n.inStream, s = n.outStream, i = function(e) {
    return function(t) {
      t ? r.emit("error", t) : s.destroyed || a.destroyed ? d > 1 && console.debug("writeFragment stream destroyed", s.destroyed, a.destroyed) : e();
    };
  }, o = i(function() {
    if (!(e = r._generateFragment(t))) return s.finalize();
    f();
  }), m = i(function() {
    var t = a.slice(e.ranges), r = s.mediaData(e.length, o);
    t.pipe(r);
  }), f = function() {
    s.destroyed || s.box(e.moof, m);
  };
  f();
};
...
MP4Remuxer.prototype.seek = function(e) {
  ...
  var r = 0, n = Math.pow(2, 53);
  t._tracks.forEach(function(a, s) {
    a.fragment = t._generateFragment(s, e);
    if (a.fragment) {
  ...
  return t._tracks.map(function(a, s) {
    ...
    if (a.fragment) {
      d && console.debug("MP4Remuxer.seek(%s)", e, a, n);
      a.inStream = new RangeSliceStream(n, {
        highWaterMark: r
      });
...
MP4Remuxer.prototype._findSampleBefore = function(e, t) {
  var r = this._tracks[e], n = Math.floor(r.timeScale * t), a = bs(r.samples, n, function(e, t) {
    return e.dts + e.presentationOffset - t;
  });
  -1 === a ? a = 0 : a < 0 && (a = -a - 2);
  if (a < 1) return 0;
  for (;!r.samples[a].sync; ) a--;
  return a;
};
...
MP4Remuxer.prototype._generateFragment = function(e, t) {
  var r, n = this._tracks[e];
  if ((r = void 0 !== t ? this._findSampleBefore(e, t) : n.currSample) >= n.samples.length) return null;
  for (var a = n.samples[r].dts, s = 0, i = [], o = r; o < n.samples.length; o++) {
    var d = n.samples[o];
    if (d.sync && d.dts - a >= n.timeScale * MIN_FRAGMENT_DURATION) break;
    s += d.size;
    var m = i.length - 1;
...
MP4Remuxer.prototype._generateMoof = function(e, t, r) {
  for (var n = this._tracks[e], a = [], s = t; s < r; s++) {
    var i = n.samples[s];
    a.push({
  ...
  var o = {
    ...
    trafs: [ {
      ...
      tfdt: {
        baseMediaDecodeTime: n.samples[t].dts
      },
      trun: {
        flags: 3841,
        dataOffset: 8,
...
Box.boxes.co64.decode = function(e, t) {
  for (var r = (e = e.slice(t)).readUInt32BE(0), n = new Array(r), a = 0; a < r; a++) {
    var s = 8 * a + 4, i = e.readUInt32BE(s), o = e.readUInt32BE(s + 4);
    n[a] = i * UINT32_MAX + o;
  }
  return {
    entries: n
...
Box.readHeaders = function(e, t, n) {
  ...
  var o, s, r = e.readUInt32BE(t), i = e.toString("ascii", t + 4, t + 8), f = t + 8;
  if (1 === r) {
    if (n - t < 16) return 16;
