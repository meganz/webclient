(mega => {
    ...
    lazy(mega.ui, 'contextMenu', () => {
        ...
        function doNodeSort() {
            const { type, dir } = this.dataset;
            M.doSort(type, parseInt(dir || '1', 10));
            M.renderMain();
        }
        const sortAscIcon = 'sprite-fm-mono icon-arrow-up-thin-outline';
        const sortDscIcon = 'sprite-fm-mono icon-arrow-down-thin-outline';
        sections.addChild('sorting', new MegaContextSection(menu, [
            {
                buttonId: 'sort-grid-item-main',
                submenu: {
                    children: [
                        {
                            buttonId: 'sort-name',
                    ...
                    dropdownText: l[6170],
                    dropdownIcon: 'sprite-fm-mono icon-queue-down'
                }
            },
        ]));
        ...
        const manipulations = {
            ...
            '.sort-grid-item-main': () => {
                const section = sections.getChild('sorting').getChild('sort-grid-item-main');
                const { n, d } = M.sortmode || {};
                section.each(comp => {
                    if (comp.dataset.type === n) {
                        comp.icon = d < 0 ? sortDscIcon : sortAscIcon;
                        comp.dataset.dir = d < 0 ? 1 : -1;
                    }
                    else {
                        comp.icon = '';
                        comp.dataset.dir = 1;
                    }
                    if (comp.dataset.type === 'ts') {
                        switch (M.currentdirid) {
                            ...
                            case 'file-requests': {
                                comp.text = l.file_request_page_label_request_created;
                                break;
                            }
                            default: {
                                comp.text = l[17445];
