...
(function() {
    ...
    var UserPresence = function userpresence(userhandle, can_webrtc, can_mobilepush, connectedcb, peerstatuscb, savesettingscb, updateuicb) {
        if (!(this instanceof userpresence)) {
            return new userpresence(userhandle);
        }

        // the four presence levels
        this.OFFLINE = 1;   // grey
        this.AWAY = 2;      // yellow
        this.ONLINE = 3;    // green
        this.DND = 4;       // red
        ...
        this.ui_presencelevel = this.ONLINE;
    ...
    UserPresence.prototype.ui_setautoaway = function presence_ui_setautoaway(timeout) {
        ...
        if (!timeout) {
            ...
            this.ui_presencelevel = this.AWAY;
        }
    ...
    UserPresence.prototype.ui_setpersist = function presence_ui_setpersist(persist) {
        if (persist != this.persist) {
            ...
            if (!persist) {
                ...
                if (this.ui_presencelevel == this.OFFLINE) {
    ...
    UserPresence.prototype.ui_signalactivity = function presence_ui_signalactivity(force) {
        ...
        if (force || !this.persist && this.autoaway && this.autoawaytimeout >= 0) {
            ...
            this.autoaway = false;
            this.sendflags(this.ui_presencelevel == this.ONLINE, this.ui_presencelevel == this.DND);
        }
    };
