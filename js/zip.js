...
function ZIPheader(filename,filesize,filetime,crc32,directory,headerpos)
{
	...
	vdescriptor.setInt32(4,crc32 ? filesize : 0,true);
	vdescriptor.setInt32(8,crc32 ? filesize : 0,true);

	header.set(descriptor,10);

	vheader.setInt16(22,filename.length,true);
	vheader.setInt16(24,0);

	var fileHeader = new Uint8Array(header.length+filename.length+4);

	new DataView(fileHeader.buffer).setInt32(0,ZIPSignature.LOCAL_FILE_HEADER,false);
	fileHeader.set(header,4);
	putString(fileHeader,filename,header.length+4);
	
	var dataDescriptor = new Uint8Array(descriptor.length+4);
	
	new DataView(dataDescriptor.buffer).setInt32(0,ZIPSignature.DATA_DESCRIPTOR);
	dataDescriptor.set(descriptor,4);
...
function ZIPsuffix(pos)
{
	...
	vzipEnd.setInt32(0,ZIPSignature.CENTRAL_DIRECTORY_END,false);
