 ...
 * $Id: videostream.js,v 4.1.1 2019/07/16 11:24:34 dc Exp $
 */
...
AudioStream.prototype.destroy = function(err) {
  if (!this.destroyed) {
    ...
    audioStream && audioStream.disconnect();
...
AudioStream.prototype._setup = function(autoplay, buffer) {
  var elm = this._elem, audioContext = this._audioContext, audioStream = audioContext.createMediaStreamDestination(), videoContext = document.createElement("canvas"), videoStream = videoContext.captureStream(30), tracks = [ audioStream.stream.getTracks()[0], videoStream.getTracks()[0] ], audioAnalyser = audioContext.createAnalyser();
...
AudioVisualiser.prototype._draw = function() {
  var self = this, tick = ++self._tick, stream = self._stream, canvas = stream._videoContext, analiser = stream._audioAnalyser, $video = $(stream._elem).parent();
...
Visualiser.prototype.draw = function(data, ctx, width, height, canvas) {
  var value, grd, r, g, b, vol = 0, i = data.byteLength;
  for (r = 0; r < 80; r++) vol += data[r];
  value = (this.volume = vol) / 1e3;
  b = g = r = 31 + (Math.sin(value) + 1);
  ctx.clearRect(0, 0, width, height);
  ctx.beginPath();
  ctx.rect(0, 0, width, height);
  (grd = ctx.createRadialGradient(width / 2, height / 2, value / 1.4, width / 2, height / 2, width - Math.min(Math.pow(value / 1.4, 2.7), width - 20))).addColorStop(0, "rgba(0,0,0,0)");
  grd.addColorStop(.8, "rgba(" + Math.round(r) + ", " + Math.round(g) + ", " + Math.round(b) + ", 0.4)");
  ctx.fillStyle = grd;
  ctx.fill();
  ctx.closePath();
  ctx.beginPath();
  ctx.fillStyle = this.gradient;
  while (i--) ctx.fillRect(5 * i, (height - (value = data[i] / 1.4)) / 1.2, 4, value / 1.3);
  ctx.closePath();
  ...
  for (r = this.stars.length; r--; ) this.stars[r].drawStar();
...
},{"/bundle/utils":3,"buffer":8,"eventemitter3":13,"to-arraybuffer":40}],2:[function(require,module,exports){
"use strict";
...
VideoFile.prototype.cachefind = function(pos) {
  ...
  for (var min = 0; ;) {
    ...
    if (pos >= this.cachepos[mid]) {
      if (pos < this.cachepos[mid] + this.cache[this.cachepos[mid]].byteLength) return mid;
      min = 1 + mid;
    } else max = mid;
...
VideoFile.prototype.fetch = function(startpos, recycle) {
  ...
  if (recycle && this.paused && this.cachesize >= this.minCache && this.canplay) d && console.debug("[VideoFile.fetch()] MIN_CACHE reached, will not fetch more data until no longer %s...", this.paused ? "paused" : "throttled"); else if (void 0 !== this.mru) {
    ...
    for (p in this.fetching) {
      if (pos >= (p *= 1) && pos < p + this.fetching[p]) return;
    ...
    length < 0 && console.error("*** Mediacache: Internal error - out of bound.", startpos, pos, length);
  }
};
...
Streamer.prototype.destroy = function() {
  ...
  if (this.video) {
    for (i = this._events.length; i--; ) this.video.removeEventListener(this._events[i], this);
...
Streamer.prototype.play = function() {
  try {
    var self = this, promise = self.video.play();
...
},{"../audiostream":1,"../videostream":43,"/bundle/utils":3,"buffer":8,"readable-stream":38}],3:[function(require,module,exports){
...
},{"/bundle/utils":3,"buffer":8,"ebml/lib/ebml/decoder":9,"eventemitter3":13,"readable-stream":38}],5:[function(require,module,exports){
...
MP4Remuxer.prototype._parseMoov = function() {
  var self = this, moov = self._moov, sidx = self._sidx;
  if (moov) {
    self._moov = self._sidx = !1;
    self._decoder && self._decoder.destroy();
    try {
...
function alloc(size, fill, encoding) {
  return size <= 0 ? createBuffer(size) : void 0 !== fill ? "string" === typeof encoding ? createBuffer(size).fill(fill, encoding) : createBuffer(size).fill(fill) : createBuffer(size);
}
...
},{"./schema.js":10,"./tools.js":11,"/bundle/utils":3,"buffer":8,"readable-stream":38}],10:[function(require,module,exports){
...
var once = require("once"), noop = function() {}, eos = function(stream, opts, callback) {
  ...
  var ws = stream._writableState, rs = stream._readableState, readable = opts.readable || !1 !== opts.readable && stream.readable, writable = opts.writable || !1 !== opts.writable && stream.writable;
  if (writable && !ws) {
...
},{"/bundle/utils":3,"readable-stream":38,"to-arraybuffer":40}],16:[function(require,module,exports){
...
},{"./descriptor":17,"./index":18,"buffer":8,"uint64be":41}],17:[function(require,module,exports){
...
},{"./boxes":16,"buffer":8,"uint64be":41}],19:[function(require,module,exports){
...
},{"/bundle/utils":3,"buffer":8,"mp4-box-encoding":18,"next-event":22,"readable-stream":38}],20:[function(require,module,exports){
...
},{"/bundle/utils":3,"buffer":8,"mp4-box-encoding":18,"readable-stream":38}],21:[function(require,module,exports){
...
},{"/bundle/utils":3,"readable-stream":38}],26:[function(require,module,exports){
"use strict";
...
var StringDecoder, createReadableStreamAsyncIterator, EE = require("eventemitter3").EventEmitter, EElistenerCount = function(emitter, type) {
...
function readableAddChunk(stream, chunk, encoding, addToFront, skipChunkCheck) {
  ...
  if (null === chunk) {
    ...
    if (er) stream.emit("error", er); else if (state.objectMode || chunk && chunk.length > 0) {
      ...
      if (addToFront) state.endEmitted ? stream.emit("error", new ERR_STREAM_UNSHIFT_AFTER_END_EVENT()) : addChunk(stream, state, chunk, !0); else if (state.ended) stream.emit("error", new ERR_STREAM_PUSH_AFTER_EOF()); else {
...
function emitReadable_(stream) {
  var state = stream._readableState;
  state.destroyed || !state.length && !state.ended || stream.emit("readable");
...
},{"../errors":26,"./_stream_duplex":27,"./internal/streams/buffer_list":32,"./internal/streams/destroy":33,"./internal/streams/state":36,"./internal/streams/stream":37,"/bundle/utils":3,"buffer":8,"eventemitter3":13,"string_decoder/":39}],30:[function(require,module,exports){
"use strict";
...
},{"../errors":26,"./_stream_duplex":27,"./internal/streams/destroy":33,"./internal/streams/state":36,"./internal/streams/stream":37,"/bundle/utils":3,"buffer":8}],32:[function(require,module,exports){
"use strict";
...
module.exports = function() {
  ...
  _proto.join = function(s) {
    if (0 === this.length) return "";
    var p = this.head, ret = "" + p.data;
    while (p = p.next) ret += s + p.data;
    return ret;
  };
  _proto.concat = function(n) {
  ...
  _proto.consume = function(n, hasStrings) {
    var ret;
...
function destroy(err, cb) {
  ...
  if (this._readableState && this._readableState.destroyed || this._writableState && this._writableState.destroyed) {
    cb ? cb(err) : !err || this._writableState && this._writableState.errorEmitted || onIdleA(emitErrorNT.bind(null, this, err));
    return this;
  }
  ...
  this._destroy(err || null, function(err) {
    if (!cb && err) {
      ...
      _this._writableState && (_this._writableState.errorEmitted = !0);
    } else if (cb) {
...
function eos(stream, opts, callback) {
  ...
  var readable = opts.readable || !1 !== opts.readable && stream.readable, writable = opts.writable || !1 !== opts.writable && stream.writable, writableEnded = stream._writableState && stream._writableState.finished, onfinish = function() {
...
module.exports = eos;
},{"../../../errors":26,"once":23}],35:[function(require,module,exports){
"use strict";
...
},{"../../../errors":26,"./end-of-stream":34,"once":23}],36:[function(require,module,exports){
"use strict";
...
},{"eventemitter3":13}],38:[function(require,module,exports){
...
},{"./lib/_stream_duplex.js":27,"./lib/_stream_passthrough.js":28,"./lib/_stream_readable.js":29,"./lib/_stream_transform.js":30,"./lib/_stream_writable.js":31,"./lib/internal/streams/end-of-stream.js":34,"./lib/internal/streams/pipeline.js":35}],39:[function(require,module,exports){
"use strict";
...
VideoStream.prototype._createMuxer = function() {
  ...
  self._muxer = {
    WebM: 1,
    Matroska: 1
  }[self._type] ? new EBMLRemuxer(self._file) : new MP4Remuxer(self._file);
...
},{"./ebml-remuxer":4,"./mp4-remuxer":5,"mediasource":15,"pump":24,"to-arraybuffer":40}]},{},[2]);
