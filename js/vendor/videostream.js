 ...
 * $Id: videostream.js,v 2.4.2 2018/01/18 20:46:19 dc Exp $
 */
...
CacheStream.prototype._read = function(e) {
  ...
  if (this._file && this._file._vs) {
    ...
    if (s > MAX_BUF_SECONDS) {
      ...
      var o = this.pos - (a = t.cachepos[a]), h = e;
      t.cache[a].byteLength - o < h && (h = t.cache[a].byteLength - o);
      if (h <= 0) break;
      e -= h;
      this.pos += h;
      t.cachemru[this.pos] = t.mru++;
      if (!this.push(Buffer.from(t.cache[a], o, h))) break;
    }
  }
};
    ...
    if (!(a < 1)) {
      ...
      this.fetcher(this.data, s, s + a).then(function(e) {
        ...
        if (void 0 !== r.mru) {
          ...
          r.cacheadd(s, t);
          s += t.byteLength;
          h === r.curfetch && setTimeout(r.fetch.bind(r, s, 1), 100);
          r.feedPlayer();
          r.backoff = 200;
          r.overquota = !1;
        }
...
Streamer.prototype.getImage = function(e, t) {
  ...
  return new Promise(function _(s, a) {
    ...
    var h = document.createElement("canvas"), n = h.getContext("2d");
    h.width = Math.round(o.width);
    h.height = Math.round(o.height);
    n.drawImage(r, 0, 0, h.width, h.height);
    for (var c, l = n.getImageData(0, 0, h.width, h.height).data, f = c = l.byteLength, u = 0; c--; ) l[c] < 10 && u++;
    ...
    } else s(dataURLToAB(h.toDataURL("image/png")));
  });
};
...
RangeSliceStream.prototype._write = function(e, t, i) {
  for (var r = !0; ;) {
    ...
    this._buffer = null;
    var s, n = this._queue[0], a = Math.max(n.start - this._position, 0), l = n.end - this._position;
    if (a >= e.length) {
    ...
    if (l > e.length) {
      this._position += e.length;
      s = 0 === a ? e : e.slice(a);
      r = n.stream.write(s) && r;
      break;
    }
    this._position += l;
    s = 0 === a && l === e.length ? e : e.slice(a, l);
    r = n.stream.write(s) && r;
    n.last && n.stream.end();
    e = e.slice(l);
    this._queue.shift();
  }
  r ? i(null) : n.stream.once("drain", i.bind(null, null));
};
...
function VideoStream(e, r, t) {
  ...
  t.sbflush || (i.flushSourceBuffers = function() {});
...
VideoStream.prototype.createWriteStream = function(e) {
  ...
  t._write = function(e, t, i) {
    if (!this.destroyed) {
      var o = this, n = this._sourceBuffer;
      if (n && !n.updating) try {
        n.appendBuffer(toArrayBuffer(e));
        this._cb = i;
        return;
      } catch (e) {
        d > 1 && console.debug("Caught %s", e.name, e);
        if ("QuotaExceededError" === e.name) r.flushSourceBuffers(-1); else if ("InvalidStateError" !== e.name) return o.destroy(e);
      }
      this._cb = function(r) {
        if (r) return i(r);
        o._write(e, t, i);
...
VideoStream.prototype.flushSourceBuffers = function(e) {
  this.forEachSourceBuffer(function(r, t, i, o, n) {
    d && console.debug("[VideoStream.flushSourceBuffers] ct=%s sr=%s er=%s", o, t, i, r.updating, n.readyState, e, r);
    if (!r.updating) {
      if (-1 !== e) i > o ? t = Math.max(o + 1, t) : o >= t && o <= i && (i = o - 1); else {
        i >= o && (i = Math.floor(o));
        if (t >= o) return;
      }
      if (i > t && i - t > 1) {
        r.remove(t, i);
        d && console.log("[VideoStream.flushSourceBuffers] remove took place", t, i);
      }
    }
  });
};
