...
ConnectionRetryManager.prototype.doConnectionRetry = function(immediately){
    ...
    //console.error("doConnectionRetry", immediately);

    if (self._$connectingPromise && self._connectionRetries >= self.options.maxConnectionRetries) {
        //console.error("self._$connectingPromise.reject", arguments);
    ...
    if (self.options.functions.isUserForcedDisconnect()) {
        //console.error("self.options.functions.isUserForcedDisconnect() -> RETURN reject");
    ...
    if (immediately !== true && self._connectionRetries > self.options.maxConnectionRetries) {
        ...
        if (self._connectionRetryInProgress) {
            //console.error("con ret. was already in progress", self._connectionRetryInProgress);
            clearTimeout(
        ...
        self._connectionRetryInProgress = setTimeout(function() {
            if (!self.options.functions.isConnected()) {
                //console.error("will try to reconnect NOW.");
                self.options.functions.reconnect(self);
            } else {
                //console.error("isConnectedOrConnecting returned true?!");
            }
        }, connectionRetryTimeout);
        //console.error("next retry in: ", connectionRetryTimeout);
...
ConnectionRetryManager.prototype._connectionRetryUI = function(){
    ...
    if (
        ...
        //console.error("Will do a forced connection retry immediately because of UI interaction.");
        self.doConnectionRetry(true);
        return true;
    }
    else {
        //console.error("Will NOT do a forced connection retry immediately because of UI interaction.");
        return false;
    }
};
