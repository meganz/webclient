 ...
 * $Id: videostream.js,v 2.4.2 2018/02/14 19:16:04 dc Exp $
 */
...
CacheStream.prototype._read = function(e) {
  ...
  if (this._file && this._file._vs) {
    ...
    if (s > MAX_BUF_SECONDS) {
      ...
      var o = this.pos - (a = t.cachepos[a]), n = e;
      t.cache[a].byteLength - o < n && (n = t.cache[a].byteLength - o);
      if (n <= 0) break;
      e -= n;
      this.pos += n;
      t.cachemru[this.pos] = t.mru++;
      if (!this.push(Buffer.from(t.cache[a], o, n))) break;
    }
  }
};
...
VideoFile.prototype.fileReader = function(e, t, i) {
  return new Promise(function(r, s) {
    var a = e.slice(t, i), o = new FileReader();
    o.onload = function() {
      r({
        buffer: o.result,
        s: e.size
...
VideoFile.prototype.fetch = function(e, t) {
  var i, r = this, s = e;
  if (t && this.paused && this.cachesize >= this.minCache && this.canplay) d && console.debug("[VideoFile.fetch()] MIN_CACHE reached, will not fetch more data until no longer %s...", this.paused ? "paused" : "throttled"); else if (void 0 !== this.mru) {
    this.curfetch += !t;
    var a;
    do {
      a = REQUEST_SIZE;
    ...
    if (!(a < 1)) {
      ...
      this.fetcher(this.data, s, s + a).then(function(e) {
        ...
        if (void 0 !== r.mru) {
          ...
          r.cacheadd(s, t);
          s += t.byteLength;
          n === r.curfetch && setTimeout(r.fetch.bind(r, s, 1), 100);
          r.feedPlayer();
          r.backoff = 200;
          r.overquota = !1;
        }
        ...
        if (void 0 !== r.mru) {
          ...
          if ("number" == typeof i) {
            if (i !== ERANGE || s < a.s) {
              d && console.warn("Unrecoverable stream fetch error, aborting...", r.isOnline, i, s);
              r.streamer.notify("error", new Error(api_strerror(i)));
            }
          } else if (509 === o.status) {
            d && console.warn("stream overquota, holding...", i);
            r.overquota = !0;
            r.retryq.push(n);
          } else {
            d && console.warn("stream fetch error, retrying...", r.isOnline, i);
            if (r.isOnline) if (o.status) {
              r.backoff = Math.min(r.backoff << 1, 7e3);
              setTimeout(n, r.backoff);
            } else n(); else r.retryq.push(n);
          }
        }
      });
      return this;
 ...
 */ function Streamer(e, t, i) {
  ...
  if (t.parentNode && "Gecko" === s.engine && parseInt(s.version) < 57) {
    r = !0;
    this._events.push("seeking");
    this.WILL_AUTOPLAY_ONSEEK = !0;
  }
  for (var a = this._events.length; a--; ) t.addEventListener(this._events[a], this, !1);
  this.video = t;
  this.timeupdate = 0;
  this.stalled = !1;
  this.inactivity = !1;
  ...
  this.file = new VideoFile(e, this);
  !1 === this.options.autoplay ? this.file.minCache = 16777216 : t.setAttribute("autoplay", !0);
  var o = {
    sbflush: r,
    bufferDuration: 1.8 * MAX_BUF_SECONDS
  };
  this.stream = new VideoStream(this.file.fetch(0), t, o);
...
Streamer.prototype.handleEvent = function(e) {
  ...
  if (this.evs[e.type]) {
    var r;
    if ("error" === e.type && !(r = this.stream._elemWrapper.detailedError)) {
      var s = t.error || !1;
      r = s.message;
      s.code && console.warn("MediaError %s", s.code, r);
    }
    this.notify(e, r || !1);
  }
};
...
Streamer.prototype._setActivityTimer = function() {
  ...
  this.activitimer = setTimeout(function() {
    if (e.stalled) {
      var t = e.video;
      if (!t.paused && !t.ended || e.file.overquota) {
        e.notify("inactivity");
        e.inactivity = !0;
      }
    }
...
Streamer.prototype.getImage = function(e, t) {
  ...
  return new Promise(function _(s, a) {
    ...
    var o = i.dim(r.videoWidth, r.videoHeight, e || 1280, t || 720);
    d && console.debug("[Streamer.getImage()] Taking %sx%s image from %sx%s at %s", o.width, o.height, r.videoWidth, r.videoHeight, secondsToTime(r.currentTime));
    var n = document.createElement("canvas"), h = n.getContext("2d");
    n.width = Math.round(o.width);
    n.height = Math.round(o.height);
    h.drawImage(r, 0, 0, n.width, n.height);
    for (var c, l = h.getImageData(0, 0, n.width, n.height).data, f = c = l.byteLength, u = 0; c--; ) l[c] < 10 && u++;
    ...
    } else s(dataURLToAB(n.toDataURL("image/png")));
  });
};
...
Streamer.getThumbnail = function(e) {
  return new Promise(function(t, i) {
    ...
    r.muted = !0;
    var s = -1, a = Streamer(e, r), o = function(e) {
      a.destroy();
      i(e);
    }, n = function(e) {
      t(e);
    ...
    a.on("playing", function() {
      if (!++s) {
        r.currentTime = 20 * (0 | r.duration) / 100;
        return !0;
      }
      a.getImage().then(n).catch(o);
    });
    a.on("error", o);
  });
};
...
MP4Remuxer.prototype._parseMoov = function() {
  var e = this, t = e._moov, r = e._sidx;
  if (t) {
    e._moov = e._sidx = !1;
    e._decoder && e._decoder.destroy();
    try {
      if (r) for (var s = t.traks.length; s--; ) {
        var a = t.traks[s];
...
MP4Remuxer.prototype._findMoov = function(e) {
  ...
  if (r.filesize >= 0 && e >= r.filesize) return t._moov ? t._parseMoov() : t.emit("error", RangeError("Offset out of bound."));
  var n = r.createReadStream({
    start: e
  });
  ...
  t._decoder.once("box", function(r) {
    var i = function() {
      t._decoder.destroy();
      t["_" + a] ? t._parseMoov() : t._findMoov(e + r.length);
    };
    n.destroy();
    d && console.debug("box", r.type, r.length, r, e);
    s[r.type] ? t._decoder.decode(function(e) {
      t["_" + r.type] = e;
      i();
    }) : i();
  });
...
MP4Remuxer.prototype._processMoov = function(e) {
  ...
  for (var a = 0; a < s.length; a++) {
    ...
    if (t._hasVideo || "vide" !== c || "avc1" !== u.type) {
      if (t._hasAudio || "soun" !== c || "mp4a" !== u.type && "fLaC" !== u.type) continue;
      if ("mp4a" === u.type) {
        n = "mp4a";
      ...
      d && console.debug(i);
      if (!(MediaSource.isTypeSupported(i) || "mp3" === n && MediaSource.isTypeSupported(i = "audio/mpeg"))) continue;
      t._hasAudio = n;
    } else {
      n = "avc1";
    ...
    if (S.value) {
      for (;;) {
        l = m.stsc.entries[y];
        var b = m.stsz.entries[h], I = S.value.duration, R = !0;
      ...
      o.tkhd.duration = 0;
      var E = l && l.sampleDescriptionId || 0, z = {
        type: "moov",
        mvhd: e.mvhd,
        traks: [ {
          tkhd: o.tkhd,
  ...
  if (0 !== t._tracks.length) {
    ...
    var O = Box.encode(t._ftyp), P = t._tracks.map(function(e) {
...
MP4Remuxer.prototype._writeFragment = function(e, t) {
  var r = this, s = r._tracks[t], a = s.inStream, n = s.outStream, i = function(e) {
    return function(t) {
      t ? r.emit("error", t) : n.destroyed || a.destroyed ? d > 1 && console.debug("writeFragment stream destroyed", n.destroyed, a.destroyed) : e();
    };
  }, o = i(function() {
    if (!(e = r._generateFragment(t))) return n.finalize();
    f();
  }), m = i(function() {
    var t = a.slice(e.ranges), r = n.mediaData(e.length, o);
    t.pipe(r);
  }), f = function() {
    n.destroyed || n.box(e.moof, m);
  };
  f();
};
...
MP4Remuxer.prototype.seek = function(e) {
  ...
  var r = 0, s = Math.pow(2, 53);
  t._tracks.forEach(function(a, n) {
    a.fragment = t._generateFragment(n, e);
    if (a.fragment) {
      var i = a.fragment.ranges[0].start;
      s = Math.min(s, i);
      r = r ? Math.abs(r - i) : i;
    }
  });
  if (d) {
    console.debug("MP4Remuxer: Measured highWaterMark to need %s bytes.", r);
    r > 1e7 && console.warn("^ excessive highWaterMark detected...");
  }
  r = r + 16777216 & -16777216;
  return t._tracks.map(function(a, n) {
    a.outStream && a.outStream.destroy();
    if (a.inStream) {
      a.inStream.destroy();
      a.inStream = null;
    }
    a.outStream = mp4.encode();
    if (a.fragment) {
      d && console.debug("MP4Remuxer.seek(%s)", e, a, s);
      a.inStream = new RangeSliceStream(s, {
        highWaterMark: r
      });
      t._fileStream || (t._fileStream = t._file.createReadStream({
        start: s
      }));
      t._fileStream.pipe(a.inStream);
      t._writeFragment(a.fragment, n);
    } else a.outStream.finalize();
    return a.outStream;
  });
};
...
MP4Remuxer.prototype._findSampleBefore = function(e, t) {
  ...
  -1 === a ? a = 0 : a < 0 && (a = -a - 2);
  if (a < 1) return 0;
  for (var n = a; !r.samples[a].sync; ) if (--a < 0) return n;
  return a;
};
...
Box.boxes.sidx.decode = function(e, t) {
  var r, s = Object.create(null), a = t + 4, n = function() {
    var t = e.readUInt16BE(a);
    a += 2;
    return t;
  }, i = function() {
...
},{"/bundle/utils":2,"binary-search":5,"buffer":6,"eventemitter3":8,"mp4-box-encoding":13,"mp4-stream":16,"range-slice-stream":20}],4:[function(require,module,exports){
"use strict";
...
Buffer.prototype.compare = function(e, t, r, n, f) {
  ...
  if (t >= r) return 1;
  if (this === e) return 0;
  for (var i = (f >>>= 0) - (n >>>= 0), o = (r >>>= 0) - (t >>>= 0), u = Math.min(i, o), s = this.slice(n, f), a = e.slice(t, r), h = 0; h < u; ++h) if (s[h] !== a[h]) {
    i = s[h];
    o = a[h];
    break;
  }
...
function arrayIndexOf(e, t, r, n, f) {
  var i, o = 1, u = e.length, s = t.length;
  ...
  if (f) {
    var a = -1;
    for (i = r; i < u; i++) if (read(e, i) === read(t, -1 === a ? 0 : i - a)) {
      -1 === a && (a = i);
    ...
    r + s > u && (r = u - s);
    for (i = r; i >= 0; i--) {
      for (var h = !0, c = 0; c < s; c++) if (read(e, i + c) !== read(t, c)) {
        h = !1;
        break;
      }
      if (h) return i;
...
var once = require("once"), noop = function() {}, eos = function(e, r, n) {
  if ("function" == typeof r) return eos(e, null, r);
  r || (r = {});
  n = once(n || noop);
  var o = e._writableState, t = e._readableState, i = r.readable || !1 !== r.readable && e.readable, l = r.writable || !1 !== r.writable && e.writable, c = function() {
    e.writable || s();
  }, s = function() {
    l = !1;
    i || n.call(e);
    ...
    return (!i || t && t.ended) && (!l || o && o.ended) ? void 0 : n.call(e, new Error("premature close"));
  }, m = function() {
    e.req.on("finish", s);
  };
  if (l && !o) {
  ...
  e.on("end", a);
  e.on("finish", s);
  !1 !== r.error && e.on("error", f);
  e.on("close", d);
  return function() {
    e.removeListener("complete", s);
    e.removeListener("abort", d);
    e.removeListener("request", m);
    e.req && e.req.removeListener("finish", s);
...
function addListener(e, t, n, r, i) {
  ...
  var s = new EE(n, r || e, i), o = prefix ? prefix + t : t;
  e._events[o] ? e._events[o].fn ? e._events[o] = [ e._events[o], s ] : e._events[o].push(s) : (e._events[o] = s, 
  e._eventsCount++);
  return e;
}
...
MediaSourceStream.prototype._write = function(e, r, t) {
  var o = this;
  if (!o.destroyed) if (o._sourceBuffer) {
    if (o._sourceBuffer.updating) return t(new Error("Cannot append buffer while source buffer updating"));
    try {
      o._sourceBuffer.appendBuffer(toArrayBuffer(e));
    } catch (e) {
      o.destroy(e);
      return;
    }
    o._cb = t;
  } else o._cb = function(a) {
    if (a) return t(a);
    o._write(e, r, t);
  };
};
...
exports.Descriptor.decode = function(r, e, o) {
  var t, c, s = r.readUInt8(e), i = e + 1, p = 0;
  do {
    p = p << 7 | 127 & (t = r.readUInt8(i++));
  } while (128 & t);
  var n = tagToName[s];
  (c = exports[n] ? exports[n].decode(r, i, o) : {
    buffer: new Buffer(r.slice(i, i + p))
  }).tag = s;
  c.tagName = n;
  c.length = i - e + p;
  c.contentsLen = p;
...
Box._encode = function(e, t, o) {
  var n = e.type, s = e.length;
  s > UINT32_MAX && (s = 1);
  t.writeUInt32BE(s, o);
  t.write(e.type, o + 4, 4, "ascii");
  var r = o + 8;
  if (1 === s) {
    uint64be.encode(e.length, t, r);
    r += 8;
  }
  if (boxes.fullBoxes[n]) {
    t.writeUInt32BE(e.flags || 0, r);
    t.writeUInt8(e.version || 0, r);
    r += 4;
  }
  if (containers[n]) {
    containers[n].forEach(function(o) {
      if (5 === o.length) {
        var n = e[o] || [];
        o = o.substr(0, 4);
    ...
    var f = boxes[n].encode;
    f(e, t, r);
    r += f.bytes;
  } else {
    if (!e.buffer) throw new Error("Either `type` must be set to a known type (not'" + n + "') or `buffer` must be set");
    e.buffer.copy(t, r);
    r += e.buffer.length;
  }
  Box.encode.bytes = r - o;
  return t;
};
...
Box.decodeWithoutHeaders = function(e, t, o, n) {
  ...
  if (containers[s]) {
    r.otherBoxes = [];
    for (var f = containers[s], i = o; n - i >= 8; ) {
  ...
  } else boxes[s] ? r = boxes[s].decode.call(e, t, o, n) : r.buffer = new Buffer(t.slice(o, n));
  r.length = e.length;
  ...
  if (containers[t]) {
    containers[t].forEach(function(t) {
      if (5 === t.length) {
        ...
        n.forEach(function(e) {
          e.type = t;
          o += Box.encodingLength(e);
        });
      } else if (e[t]) {
        var s = e[t];
        s.type = t;
        o += Box.encodingLength(s);
      }
    });
    e.otherBoxes && e.otherBoxes.forEach(function(e) {
      o += Box.encodingLength(e);
    });
  } else if (boxes[t]) o += boxes[t].encodingLength(e); else {
...
var Buffer = require("buffer").Buffer, stream = require("readable-stream"), inherits = require("/bundle/utils").inherit, nextEvent = require("next-event"), Box = require("mp4-box-encoding"), EMPTY = new Buffer(0);
...
Decoder.prototype._write = function(e, t, i) {
  if (!this.destroyed) {
    for (var r = !this._str || !this._str._writableState.needDrain; e.length && !this.destroyed; ) {
      ...
      this._missing -= s;
      if (!this._missing) {
        var n = this._buf, o = this._cb, h = this._str;
...
Readable.prototype.read = function(e) {
  ...
  if (t.ended || t.reading) debug("reading or ended", i = !1); else if (i) {
    debug("do read");
    t.reading = !0;
    t.sync = !0;
...
Readable.prototype.wrap = function(e) {
  ...
  e.on("data", function(i) {
    ...
    if ((!n.objectMode || null != i) && (n.objectMode || i && i.length) && !t.push(i)) {
      r = !0;
      e.pause();
    }
  });
  ...
  for (var a = 0; a < kProxyEvents.length; a++) e.on(kProxyEvents[a], this.emit.bind(this, kProxyEvents[a]));
...
},{"./_stream_duplex":21,"./internal/streams/BufferList":26,"./internal/streams/destroy":27,"./internal/streams/stream":28,"/bundle/utils":2,"buffer":6,"eventemitter3":8,"string_decoder/":30}],24:[function(require,module,exports){
"use strict";
...
Writable.prototype.write = function(e, t, i) {
  var r = this._writableState, n = !1, s = !r.objectMode && _isUint8Array(e);
...
function destroy(t, e) {
  ...
  if (this._readableState && this._readableState.destroyed || this._writableState && this._writableState.destroyed) {
    e ? e(t) : !t || this._writableState && this._writableState.errorEmitted || processNextTick(emitErrorNT, this, t);
    return this;
  }
...
},{"eventemitter3":8}],29:[function(require,module,exports){
...
function VideoStream(e, r, t) {
  ...
  t = t || {};
  o.detailedError = null;
  o._elem = r;
  o._elemWrapper = new MediaElementWrapper(r, t);
  o._waitingFired = !1;
  o._trackMeta = null;
  o._file = e;
  o._tracks = null;
  "none" !== o._elem.preload && o._createMuxer();
  t.sbflush || (o.flushSourceBuffers = function() {});
  o._onError = function(e) {
    o.detailedError = o._elemWrapper.detailedError || e;
    d && console.error("VideoStream Error.", e, o.detailedError);
    o.destroy();
  };
  o._onWaiting = function() {
    o._waitingFired = !0;
    o._muxer ? o._tracks && o._pump() : o._createMuxer();
  };
  o._elem.addEventListener("waiting", o._onWaiting);
  o._elem.addEventListener("error", o._onError);
}
...
VideoStream.prototype._createMuxer = function() {
  ...
  e._muxer.on("ready", function(r) {
    e._tracks = r.map(function(r) {
      var t = e.createWriteStream(r.mime), o = {
        muxed: null,
        mediaSource: t,
        initFlushed: !1,
        onInitFlushed: null
      ...
      t.write(r.init, function(e) {
        o.initFlushed = !0;
        o.onInitFlushed && o.onInitFlushed(e);
      });
      return o;
    });
...
VideoStream.prototype.createWriteStream = function(e) {
  var r = this, t = r._elemWrapper.createWriteStream(e), o = t.destroy;
  t._write = function(e, o, i) {
    if (!this.destroyed) {
      var n = this, u = this._sourceBuffer;
      if (u && !u.updating) try {
        if (d > 6) {
          if (!u._mimeType) {
            u._mimeType = t._type;
            u.addEventListener("abort", console.warn.bind(console));
            u.addEventListener("error", console.warn.bind(console));
          }
          console.warn("Appending buffer to media source " + u._mimeType, [ e ], u, this._mediaSource.readyState);
        }
        u.appendBuffer(toArrayBuffer(e));
        this._cb = i;
        return;
      } catch (e) {
        d > 1 && console.debug("Caught %s", e.name, e);
        if ("QuotaExceededError" !== e.name) return n.destroy(e);
        r.flushSourceBuffers(-1);
      }
      this._cb = function(r) {
        if (r) return i(r);
        n._write(e, o, i);
  ...
  t.destroy = function(e) {
    try {
      ...
      var r = this, i = this._sourceBuffer;
      d && console.debug('Caught exception ("%s")', t.name, i && i.updating, i, t);
      if (!i || !i.updating) throw t;
      e && r.emit("error", e);
      r.emit("close");
    }
  };
...
VideoStream.prototype._tryPump = function() {
  var e = this, r = e._elem.currentTime, t = e._muxer.seek(r, !e._tracks);
  e._tracks.forEach(function(o, i) {
    var n = function() {
      if (o.muxed) {
        var n = o.mediaSource || !1;
        "audio/mpeg" === n._type && n._sourceBuffer && (n._sourceBuffer.timestampOffset = r);
        o.muxed.destroy();
        o.mediaSource = e.createWriteStream(o.mediaSource);
      }
      o.muxed = t[i];
      pump(o.muxed, o.mediaSource);
    };
    o.initFlushed ? n() : o.onInitFlushed = function(r) {
      r ? e._elemWrapper.error(r) : n();
    };
  });
};
...
VideoStream.prototype.forEachSourceBuffer = function(e) {
  if (this._tracks) {
    var r, t, o, i, n, u = this._elem.currentTime;
    for (r = this._tracks.length; r--; ) {
      n = this._tracks[r].mediaSource._mediaSource;
      t = this._tracks[r].mediaSource._sourceBuffer;
      try {
        o = t.buffered.length ? t.buffered.start(0) : 0;
        i = t.buffered.length ? t.buffered.end(t.buffered.length - 1) : 0;
        e.call(this, t, o, i, u, n);
      } catch (e) {
        console.debug(e);
      }
    }
...
VideoStream.prototype.flushSourceBuffers = function(e) {
  this.forEachSourceBuffer(function(r, t, o, i, n) {
    d && console.debug("[VideoStream.flushSourceBuffers] ct=%s sr=%s er=%s", i, t, o, r.updating, n.readyState, e, r);
    if (!r.updating) {
      if (-1 !== e) o > i ? t = Math.max(i + 1, t) : i >= t && i <= o && (o = i - 1); else {
        o >= i && (o = Math.floor(i));
        if (t >= i) return;
      }
      if (o > t && o - t > 1) {
        r.remove(t, o);
        d && console.log("[VideoStream.flushSourceBuffers] remove took place", t, o);
      }
    }
  });
};
