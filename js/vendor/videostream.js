 ...
 * $Id: videostream.js,v 2.4.2 2018/01/23 17:28:44 dc Exp $
 */
...
MP4Remuxer.prototype._processMoov = function(e) {
  ...
  for (var n = 0; n < r.length; n++) {
    var s, a, i = r[n], o = i.mdia.minf.stbl, m = o.stco || o.co64, f = o.stsd.entries[0], c = i.mdia.hdlr.handlerType;
    d && console.debug("handler=%s, type=%s, trak:", c, f.type, i);
    if ("vide" === c && "avc1" === f.type) {
      if (t._hasVideo) continue;
      t._hasVideo = !0;
      s = "avc1";
      f.avcC && (s += "." + f.avcC.mimeCodec);
      a = 'video/mp4; codecs="' + s + '"';
      d && console.debug(a);
    } else {
      if ("soun" !== c || "mp4a" !== f.type) continue;
      if (t._hasAudio) continue;
      s = "mp4a";
      f.esds && f.esds.mimeCodec && (s += "." + f.esds.mimeCodec);
      a = 'audio/mp4; codecs="' + s + '"';
      d && console.debug(a);
  ...
  if (0 !== t._tracks.length) {
    ...
    var w = Box.encode(t._ftyp), z = t._tracks.map(function(e) {
...
MP4Remuxer.prototype._writeFragment = function(e, t) {
  var r = this, n = r._tracks[t], s = n.inStream, a = n.outStream, i = function(e) {
    return function(t) {
      t ? r.emit("error", t) : a.destroyed || s.destroyed ? d > 1 && console.debug("writeFragment stream destroyed", a.destroyed, s.destroyed) : e();
    };
  }, o = i(function() {
    if (!(e = r._generateFragment(t))) return a.finalize();
    f();
  }), m = i(function() {
    var t = s.slice(e.ranges), r = a.mediaData(e.length, o);
    t.pipe(r);
  }), f = function() {
...
MP4Remuxer.prototype.seek = function(e) {
  ...
  var r = Math.pow(2, 53);
  t._tracks.forEach(function(n, s) {
    n.fragment = t._generateFragment(s, e);
    n.fragment && (r = Math.min(r, n.fragment.ranges[0].start));
  });
  return t._tracks.map(function(n, s) {
    ...
    if (n.fragment) {
      d && console.debug("MP4Remuxer.seek(%s)", e, n, r);
      n.inStream = new RangeSliceStream(r, {
        highWaterMark: 1e7
      });
...
MP4Remuxer.prototype._findSampleBefore = function(e, t) {
  var r = this._tracks[e], n = Math.floor(r.timeScale * t), s = bs(r.samples, n, function(e, t) {
    return e.dts + e.presentationOffset - t;
  });
  -1 === s ? s = 0 : s < 0 && (s = -s - 2);
  if (s < 1) return 0;
  for (;!r.samples[s].sync; ) s--;
  return s;
};
...
MP4Remuxer.prototype._generateFragment = function(e, t) {
  ...
  for (var s = n.samples[r].dts, a = 0, i = [], o = r; o < n.samples.length; o++) {
    var d = n.samples[o];
    if (d.sync && d.dts - s >= n.timeScale * MIN_FRAGMENT_DURATION) break;
    a += d.size;
    var m = i.length - 1;
...
MP4Remuxer.prototype._generateMoof = function(e, t, r) {
  for (var n = this._tracks[e], s = [], a = t; a < r; a++) {
    var i = n.samples[a];
    s.push({
...
Box.boxes.co64.decode = function(e, t) {
  for (var r = (e = e.slice(t)).readUInt32BE(0), n = new Array(r), s = 0; s < r; s++) {
    var a = 8 * s + 4, i = e.readUInt32BE(a), o = e.readUInt32BE(a + 4);
    n[s] = i * UINT32_MAX + o;
  }
  return {
    entries: n
