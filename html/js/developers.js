...
function dev_movemenu()
{
    ...
    if (page === 'developers') {
        ...
        if ($('body').scrollTop() > 110) {
            ...
            if ((currenttop !== top) && (!dev_moving)) {
                dev_domovemenu(0,currenttop,(top-currenttop),20);
            }
        } else if ((currenttop !== 0) && (!dev_moving)) {
            dev_domovemenu(0,currenttop,(0-currenttop),20);
        }
...
function dev_init(sub,reload)
{
    ...
    $(".account-link").bind('click', function(e)
    {
        if ($(this).attr('class').indexOf('selected') === -1) {
            ...
            if ($(this).next().attr('class').indexOf('dev-new-submenu') !== -1) {
...
function dev_renderapps()
{
    ...
    {
        if (!sdk_apps.hasOwnProperty(key)) {
            continue;
        }
        var status = 'Development';
        if (sdk_apps[key].status === 1) {
            status = 'Operational';
        }
        if (sdk_apps[key]) {
    ...
    if (i === 0) {
        apps_table += '<tr><td colspan="3"><i>You have no apps created yet.</i></td></tr>';
    }
