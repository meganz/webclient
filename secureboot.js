...
else if (!b_u)
{
    ...
    {
        if (window.URL) evalscript_url([asmCryptoSha256]);
        else evalscript(asmCryptoSha256);
    }
    if ((typeof Worker !== 'undefined') && (typeof window.URL !== 'undefined') && !fx_startup_cache && !nocontentcheck)
    {
        var hashdata = ['self.postMessage = self.webkitPostMessage || self.postMessage;', asmCryptoSha256, 'self.onmessage = function(e) { try { var hashHex = asmCrypto.SHA256.hex(e.data.text); e.data.hash = hashHex; self.postMessage(e.data); } catch(err) { e.data.error = err.message; self.postMessage(e.data);  } };'];
        ...
        xhr_stack[xhri].onload = function()
        {
            ...
            else
            {
                if (!nocontentcheck) {
                    ...
                    var hashHex = asmCrypto.SHA256.hex(jsl[this.jsi].text);
