...
window.CanvasCropper = (function() {
    ...
    CanvasCropper.prototype.crop = function(image, sourceX, sourceY, cropWidth, cropHeight) {
        ...
        context.drawImage(image,
            sourceX, sourceY, cropWidth, cropHeight, targetX, targetY, 250, 250);
...
window.ClientFileHandler = (function() {
    ...
    ClientFileHandler.prototype.filterFiles = function(fileList) {
        var fileTypeFilter = _.isRegExp(this.options.fileTypeFilter)
            ? this.options.fileTypeFilter : this.defaults.fileTypeFilter,
...
window.ClientFileReader = (function() {
    ...
    ClientFileReader.prototype.onSuccess = function(files) {
        var readMethod = _.has(_readMethodMap,
            this.options.readMethod) ? _readMethodMap[this.options.readMethod] : undefined;

        if (readMethod) {
            _.each(files, _.bind(function(file) {
                var fileReader = new FileReader();
                fileReader.onload = _.bind(this.readFile,
                    this,
                    file); //pass the file handle to allow callback access to filename, size, etc.
    ...
    ClientFileReader.prototype.readFile = function(file, fileReaderEvent) {
        _.isFunction(this.options.onRead)
            && this.options.onRead(fileReaderEvent.target.result, file);
    };
...
window.DragDropFileTarget = (function() {
    ...
    DragDropFileTarget.prototype.onDrop = function(e) {
        ...
        if (this.options.clientFileHandler) {
            this.options.clientFileHandler.handleFiles(e.originalEvent.dataTransfer.files,
...
window.ImageExplorer = (function() {
    ...
    ImageExplorer.prototype.init = function($container, opts) {
        ...
        _.bindAll(this,
            'getImageSrc',
            'setImageSrc', 'initImage', 'initDragDelegate', 'initScaleSlider', 'setInitialScale',
            'getFillScale',
            'getContainedScale', 'getCircularContainedScale', 'sliderValToScale', 'scaleToSliderVal',
            'updateImageScale',
            'resetImagePosition',
            'resetScaleSlider', 'toggleEmpty', 'get$ImageView', 'get$SourceImage',
            'get$Mask',
            'get$DragDelegate',
            'getMaskedImageProperties', 'showError', 'clearError', 'hasValidImage',
    ...
    ImageExplorer.prototype.setImageSrc = function(src) {
        if (this.$sourceImage) {
            ...
            //Force image to reset if the user uploads the same image
        }
    };
    ...
    ImageExplorer.prototype.setInitialScale = function() {
        ...
        switch (this.options.initialScaleMode) {
            case ImageExplorer.scaleModes.fill:
                ...
                this.minScale = initialScale = this.getFillScale(naturalWidth,
                    naturalHeight, maskWidth, maskHeight);
                break;
            ...
            case ImageExplorer.scaleModes.contain:
                ...
                if (this.$mask.hasClass('circle-mask')) {
                    this.minScale = initialScale = this.getCircularContainedScale(naturalWidth,
                        naturalHeight, maskWidth / 2);
                }
                else {
                    this.minScale = initialScale = this.getContainedScale(naturalWidth,
                        naturalHeight, maskWidth, maskHeight);
                }
                break;
            ...
            case ImageExplorer.scaleModes.containAndFill:
                ...
                if (this.$mask.hasClass('circle-mask')) {
                    this.minScale = this.getCircularContainedScale(naturalWidth,
                        naturalHeight, maskWidth / 2);
                }
                else {
                    this.minScale = this.getContainedScale(naturalWidth,
                        naturalHeight, maskWidth, maskHeight);
                }

                initialScale = this.getFillScale(naturalWidth,
                    naturalHeight, maskWidth, maskHeight);
                break;
        }
    ...
    ImageExplorer.prototype.getFillScale = function(imageWidth,
        imageHeight, constraintWidth, constraintHeight) {
        var widthRatio = constraintWidth / imageWidth,
            heightRatio = constraintHeight / imageHeight;
        return Math.max(widthRatio, heightRatio);
    };

    ImageExplorer.prototype.getContainedScale = function(imageWidth,
        imageHeight, constraintWidth, constraintHeight) {
        var widthRatio = constraintWidth / imageWidth,
            heightRatio = constraintHeight / imageHeight;
    ...
    ImageExplorer.prototype._resetFromError = function() {
        ...
        _.isFunction(this.options.onErrorReset) && this.options.onErrorReset(hasValidImage
            ? this.getImageSrc()
            : undefined);
    };
...
window.ImageUploadAndCrop = (function() {
    ...
    ImageUploadAndCrop.prototype.init = function($container, opts) {
        ...
        _.bindAll(this,
            'crop',
            'resetState',
            '_onFileProcessed', 'setImageSrc', 'validateImageResolution', '_onFilesError',
            ...
            if (!isNaN(this.options.maxImageDimension)) {
                ...
                validatePromise
                    ...
                    .fail(_.bind(function(imageWidth, imageHeight) {
                        this._onFileError('The selected image size is ' + imageWidth + 'px * ' +
                            imageHeight + 'px. The maximum allowed image size is ' +
                            this.options.maxImageDimension + 'px * ' +
                            this.options.maxImageDimension + 'px');
                    }, this));
            }
            else {
    ...
    ImageUploadAndCrop.prototype.validateImageResolution = function(imageSrc) {
        ...
        tmpImage.onload = function() {
            if (this.naturalWidth > self.options.maxImageDimension
                    || this.naturalHeight > self.options.maxImageDimension) {
    ...
    ImageUploadAndCrop.prototype._onFilesError = function(invalidFiles) {
        ...
        if (invalidFiles && invalidFiles.bySize && invalidFiles.bySize.length) {
            ...
            this._onFileError('File "' + str_mtrunc(file.name, 50)
                + '" is ' + bytesToSize(file.size)
                + ' which is larger than the maximum allowed size of '
                + bytesToSize(this.options.fileSizeLimit));
        }
        else {
