...
function RtcModule(chatd, crypto, handler, iceServers) {
    ...
    this.chatd = chatd;
    this.crypto = crypto;
    this.ownAnonId = crypto.ownAnonId();
...
Call.prototype._onUserOffline = function(userid, clientid) {
    ...
    for (var sid in this.sessions) {
        ...
        if (sess.peer === userid && sess.peerClient === clientid) {
            setTimeout(function() {
                sess.terminateAndDestroy(Term.kErrNetSignalling | Term.kPeer);
            }, 0);
            return true;
        }
    }
...
Session.prototype.msgSdpOfferSendAnswer = function(packet) {
    ...
    .catch(function(err) {
        ...
        var msg = err.remote
                ? ("Error accepting remote SDP offer: " + err.err)
                : ("Error creating SDP answer: " + err.err);
...
Session.prototype.sid = function() {
    return this.peer + this.peerClient;
};
...
var Term = Object.freeze({
    ...
    kErrorLast: 32,         // < Last enum indicating call termination due to error
    kLast: 32,              // < Last call terminate enum value
